...
export default function MemoryPage(_: Props) {
  ...
  async function save() {
    ...
    if (isNew) {
      ...
      await memoryService.update(selected.id, data);
  ...
  async function handleArchive(m: MemoryRecord) {
    await memoryService.archive(m.id);
    setSelected(null);
    refresh();
  }

  async function handleDelete(m: MemoryRecord) {
    await memoryService.delete(m.id);
    setSelected(null);
    refresh();
  }
        ...
        <div className="mem-list">
          {memories.length === 0 && (
            <div className="mem-empty">
              {query ? 'No results.' : showArchived ? 'No archived memories.' : 'No memories yet — create one.'}
            </div>
          )}
          {memories.map(m => (
            <button
      ...
      <main className="mem-main">
        ...
        {!editing && !selected && (
          <div className="mem-placeholder">
            ...
            <p className="mem-placeholder-text">Select a memory or create a new one.</p>
          </div>
        )}
